function followRow()
{
    ...
    sessionStorage.setItem('customerGroupKey',this.cells[1].innerText)
    x = sessionStorage.getItem('customerGroupKey')
    console.log(x)
    window.location = './customerGroupUpdate.html'
}
...
function showDatabase()
{
    ...
    .then(data => 
    {
        ...
        for(i=0; i<limit; i++)
        {
            ...
            element2.innerHTML = data.CustomerGroup[rowCount-1].customerGroup
            element2.style.textAlign = "centre"
            cell1.appendChild(element2);
        
            var cell2 = row.insertCell(2)
            var element3 = document.createElement("span")
            element3.innerHTML = data.CustomerGroup[rowCount-1].dealsIn
            element3.style.textAlign = "centre"
            cell2.appendChild(element3);
        
            var cell3 = row.insertCell(3)
            var element4 = document.createElement("span")
            element4.innerHTML = data.CustomerGroup[rowCount-1].poc
            element4.style.textAlign = "centre"
            cell3.appendChild(element4);

            var cell4 = row.insertCell(4)
            var element5 = document.createElement("span")
            element5.innerHTML = data.CustomerGroup[rowCount-1].emailId
            element5.style.textAlign = "centre"
            cell4.appendChild(element5);

            var cell5 = row.insertCell(5)
            var element6 = document.createElement("span")
            element6.innerHTML = data.CustomerGroup[rowCount-1].phoneNo
            element6.style.textAlign = "centre"
            cell5.appendChild(element6);

            var cell6 = row.insertCell(6)
            var element7 = document.createElement("span")
            element7.innerHTML = data.CustomerGroup[rowCount-1].mobileNo
            element7.style.textAlign = "centre"
            cell6.appendChild(element7);

            var cell7 = row.insertCell(7)
            var element8 = document.createElement("span")
            element8.innerHTML = data.CustomerGroup[rowCount-1].line1 + "   " + data.CustomerGroup[rowCount-1].line2 + "   " + data.CustomerGroup[rowCount-1].city + "   " + data.CustomerGroup[rowCount-1].state + "   " + data.CustomerGroup[rowCount-1].pinCode
...
document.getElementById('back').onclick = () => { 
    ...
    if(y==null)
    {            
        window.location = "./customerGroupHome.html"
    }
}